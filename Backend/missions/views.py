...
from .serializers import (
    ...
    VehiculesSerializer,TrajetsSerializer, VehiculeLouesSerializer,RecetteGlobleSerializer,
     DepenseMissionsSerializer, listAcceuilMissionExerciceSerializer,InfosRecetteCoutUnitaire)
...
@api_view(['GET'])
def recetteInfoCoutUnitaire(request,id_chaine=None,*args, **kwargs):
    
    queryset = Recettes.objects.all()

    if(id_chaine is not None and len( tuple( id_chaine.split(",") ) )):
        
        list_id = tuple(id_chaine.split(","))
        queryset = queryset.filter(produit=list_id[0],mission=list_id[1],
                                client_concerne=list_id[2]).values("cout_unitaire", "id")
        return Response(queryset)
    return None
recetteInfoCoutUnitaireAPIView = recetteInfoCoutUnitaire