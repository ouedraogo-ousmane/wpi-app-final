...
from missions.models import Chauffeurs, VehiculeParcs
...
class MantenanceSerializer(serializers.ModelSerializer):
    ...
    vehiculeData = serializers.SerializerMethodField()
    class Meta:
        model = Mantenances
        fields = (
            'id',
    ...
    def get_chauffeur(self, obj):
        ...
        return chauffeur.values("nom", "prenom")[0]
    def get_vehiculeData(self, obj):
        vehicule = obj.vehiculeConcerne
        #chauffeur  = Chauffeurs.objects.filter(vehicule=vehicule)
        return VehiculeParcs.objects.filter(pk=vehicule).values("id","immat")[0]


class MantenanceSendSerializer(serializers.ModelSerializer):
    '''
        Serializer pour l'activite des maintenance
    '''
