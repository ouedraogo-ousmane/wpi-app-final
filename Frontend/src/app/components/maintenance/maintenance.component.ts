...
export class MaintenanceComponent implements OnInit,AfterViewInit  {
  ...
  displayedColumns: string[] = ['select', 'motif', 'chauffeur','date_maintenance', 'montant','actions'];
  ...
  IsWait : boolean = true;
  pieceSelected!: Piece;
  maintenanceSelected: Maintenances =  new Maintenances();
  ...
  next : string ="";
  previous : string ="";
  urlMaint :string="http://127.0.0.1:8000/maintenances/";
  ...
  urlMaintenance = 'http://127.0.0.1:8000/admin/maintenances/mantenances/';
 ...
 */
  ...
  clickedRow(row:Maintenances){
    if(this.selection.isSelected(row) ===true){
      ...
      this.maintenanceSelected =  this.selection.selected[0];
      console.log(this.maintenanceSelected);
    }
  }
 ...
 */
  ...
  getTotalMontant(data:{nomPiece:{id:number,nom:string},coutUnitaire:number,nombre:number}[]){
    let somme =0;
    for(let d of data){
      somme += Number(d.coutUnitaire)*Number(d.nombre);
   ...
   */
    ...
    if(this.next.length !=0){
      this.getAllMaintenance(this.next);
    }
  }
  getPreviousPage(){
    if(this.previous.length!=0){
      this.getAllMaintenance(this.previous);
    }
  }
