...
import { InfoPesee, Itrajets } from '../programmer/iprogrammer';
...
//trajetsList/<int:pk>/detail/
updateTrajet(data:Itrajets):Observable<Itrajets>{
  const endpoint:string = "http://127.0.0.1:8000/missions/trajetsList/"+data.id+"/detail/"
  return this.http.patch<Itrajets>(endpoint, data, this.httpOptions)
};

//infoCoutProduit/<str:id_chaine>/detail/
infoProduitCout(id_chaine:string):Observable<{cout_unitaire:number, id:number}[]>{
  const endpoint:string = "http://127.0.0.1:8000/missions/infoCoutProduit/"+id_chaine+"/detail/"
  return this.http.get<{cout_unitaire:number, id:number}[]>(endpoint,this.httpOptions);
}
}
